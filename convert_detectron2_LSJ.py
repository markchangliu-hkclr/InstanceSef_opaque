...
def convert(src, dst):
    ...
    # sample_model = torch.load('/home/xjgao/InstanceSeg/checkpoints/mask_rcnn_r50_fpn_mstrain-poly_3x_coco_20210524_201154-21b550bb.pth')
    sample_model = torch.load('/home/xjgao/InstanceSeg/checkpoints/model_final_14d201-mmdet.pth')

    dst_state_dict = OrderedDict()
    for k, v in src_model['model'].items():
        key_name_split = k.split('.')
        if 'backbone.fpn_lateral' in k:
            if key_name_split[-2]=='norm':
                lateral_id = int(key_name_split[-3][-1])
            ...
            if 'conv' in key_name_split[2]:
                name = f'rpn_head.rpn_conv.{key_name_split[-2][-1]}.conv.{key_name_split[-1]}'
            ...
            if key_name_split[1] == 'box_head':
                if key_name_split[2][:-1] == 'conv':
                    conv_id = int(key_name_split[2][-1]) - 1
                    if 'norm' in k:
                        name = f'roi_head.bbox_head.shared_convs.{conv_id}.bn.{key_name_split[-1]}'
                    else:
                        name = f'roi_head.bbox_head.shared_convs.{conv_id}.conv.{key_name_split[-1]}'
                else:
...
def main():
    ...
    parser.add_argument('--src', default='/home/xjgao/InstanceSeg_detectron2/checkpoints/model_final_14d201.pkl', help='src detectron model path')
    parser.add_argument('--dst', default='/home/xjgao/InstanceSeg/checkpoints/model_final_14d201-mmdet.pth', help='save path')
